...
auto Matt_Study::Reader::operator()(const std::string& file) -> void {
    ...
    while (reader.next(hipo_event, REC_Particle, REC_Event)) {
        if (REC_Particle.isEmpty())
            continue;
        ...
        if (cutChi2_e)
            fHistograms.electronKinematics.hist1D_vz_cut->Get()->Fill(vz_e);
        if (cutVz_e)
            fHistograms.electronKinematics.hist1D_chi2_cut->Get()->Fill(chi2_e);
        ...
        if (good_pionsMinus.size() < 1 || good_pionsPlus.size() < 1)
            continue;
        ...
        fCounter.nb_good_electron_with_good_pp_and_pm++;
        for (auto& pp : good_pionsPlus)
            fCounter.nb_good_pp_with_good_electron_and_pm++;
        for (auto& pp : good_pionsMinus)
            fCounter.nb_good_pm_with_good_pp_and_electron++;
        ...
        for (auto& [pionPlus, pionMinus] : all_combinaison) {
            ...
            fHistograms.eventKinematics.hist1D_DeltaE->Get()->Fill(event.DeltaE());
            ...
            if (cut_w && cut_zh && cut_t && cut_lc) {
                fHistograms.eventKinematics.hist1D_DeltaE_cut->Get()->Fill(event.DeltaE());
            }
            ...
            if (cuts) {
                ...
                fHistograms.eventKinematics.hist1D_electron_p_aftercut->Get()->Fill(electron.P());
                fHistograms.eventKinematics.hist1D_electron_phi_aftercut->Get()->Fill(electron.Phi());
                fHistograms.eventKinematics.hist1D_electron_theta_aftercut->Get()->Fill(electron.Theta());
                fHistograms.eventKinematics.hist1D_electron_vz_aftercut->Get()->Fill(electron.Vz());
                fHistograms.eventKinematics.hist1D_electron_chi2_aftercut->Get()->Fill(electron.ChiSquare());

                fHistograms.eventKinematics.hist1D_pion_plus_p_aftercut->Get()->Fill(pionPlus.P());
                fHistograms.eventKinematics.hist1D_pion_plus_phi_aftercut->Get()->Fill(pionMinus.Phi());
                fHistograms.eventKinematics.hist1D_pion_plus_theta_aftercut->Get()->Fill(pionMinus.Theta());
                fHistograms.eventKinematics.hist1D_pion_plus_vz_aftercut->Get()->Fill(pionMinus.Vz());
                fHistograms.eventKinematics.hist1D_pion_plus_chi2_aftercut->Get()->Fill(pionMinus.ChiSquare());

                fHistograms.eventKinematics.hist1D_pion_minus_p_aftercut->Get()->Fill(pionMinus.P());
                fHistograms.eventKinematics.hist1D_pion_minus_phi_aftercut->Get()->Fill(pionMinus.Phi());
                fHistograms.eventKinematics.hist1D_pion_minus_theta_aftercut->Get()->Fill(pionMinus.Theta());
...
auto Matt_Study::Reader::FindTopology(const hipo::bank& REC_Particle) -> std::optional<Topology> {
    ...
    if (!electronFind || !pionPlusFind || !pionMinusFind)
        return std::nullopt;
    ...
    for (int i = 0; i < REC_Particle.getRows(); i++) {
        ...
        if (pid == 211)
            pionsPlus.emplace_back(pid, status, i, px, py, pz, E, vx, vy, vz, vt, beta, chiSquare);
        if (pid == -211)
            pionsMinus.emplace_back(pid, status, i, px, py, pz, E, vx, vy, vz, vt, beta, chiSquare);
    }

    if (electron.PdgCode() == 0)
        return std::nullopt;

    //if (!(pionsPlus.size() > 0 && pionsMinus.size() > 0)) return std::nullopt;
    if (pionsPlus.size() < 1 && pionsMinus.size() < 1)
        return std::nullopt;
