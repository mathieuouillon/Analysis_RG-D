...
auto DrawHist1D(const std::shared_ptr<TH1> &hNoCut, const std::shared_ptr<TH1> &hWithCut, const std::string &path, const ArgsHist1D &args = {}) -> void {
        ...
        if (args.fScaled && scale > 5) hWithCutCopy->Scale(scale);
        ...
        if (args.fScaled && scale > 2) Helper::DrawScaleText(scale);
        ...
        if (args.fSetStats) Helper::SetStatBoxes(canvas, args.fFitFunc != nullptr, hNoCut, hWithCutCopy);
        Helper::SaveCanvas(canvas, path, myFileName);
    }
